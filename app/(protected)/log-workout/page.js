...
export default function LogWorkoutView() {
  ...
  // Always render the Suspense/SearchParamHandler boundary to set templateId
  // (before any early returns)
  return (
    <>
      <Suspense fallback={null}>
        <SearchParamHandler param="templateId" onResult={setTemplateId} />
      </Suspense>
      {loading ? (
          ...
          <div className="pb-52">
            <form className="space-y-6 mt-4">
              {["warmup", "main", "cooldown"].map((section) => {
                const exercises = formData.filter(
